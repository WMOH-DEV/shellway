...
const api = {
  ...
  theme: {
    getNative: () =>
      ipcRenderer.invoke("theme:getNative") as Promise<"dark" | "light">,
  },
  ...
  sessions: {
    ...
    deleteMany: (ids: string[]) =>
      ipcRenderer.invoke("session:deleteMany", ids),
    touch: (id: string) => ipcRenderer.invoke("session:touch", id),
    reorder: (orderedIds: string[]) =>
      ipcRenderer.invoke("session:reorder", orderedIds),
    getGroups: () =>
      ipcRenderer.invoke("session:getGroups") as Promise<string[]>,
    setGroups: (groups: string[]) =>
      ipcRenderer.invoke("session:setGroups", groups),
    export: () => ipcRenderer.invoke("session:export"),
    import: (sessions: unknown[]) =>
      ipcRenderer.invoke("session:import", sessions),
    ...
    exportBuild: (options: Record<string, unknown>) =>
      ipcRenderer.invoke("export:build", options) as Promise<{
        success: boolean;
        data?: string;
        error?: string;
      }>,
  ...
  fs: {
    ...
    watchFile: (watchId: string, filePath: string) =>
      ipcRenderer.invoke("fs:watchFile", watchId, filePath) as Promise<{
        success: boolean;
      }>,
    unwatchFile: (watchId: string) =>
      ipcRenderer.invoke("fs:unwatchFile", watchId) as Promise<{
        success: boolean;
      }>,
  ...
  shell: {
    ...
    openWithPicker: (filePath: string) =>
      ipcRenderer.invoke("shell:openWithPicker", filePath) as Promise<{
        appPath: string;
        appName: string;
      } | null>,
  },
  ...
  ssh: {
    ...
    onStatusChange: (
      callback: (connectionId: string, status: string) => void,
    ) => {
      const handler = (
        _e: Electron.IpcRendererEvent,
        id: string,
    ...
    onReconnectSuccess: (
      callback: (connectionId: string, attempt: number) => void,
    ) => {
      const handler = (
        _e: Electron.IpcRendererEvent,
        id: string,
    ...
    onReconnectFailed: (
      callback: (connectionId: string, attempt: number, error: string) => void,
    ) => {
      const handler = (
        _e: Electron.IpcRendererEvent,
        id: string,
    ...
    onReconnectExhausted: (
      callback: (connectionId: string, totalAttempts: number) => void,
    ) => {
      const handler = (
        _e: Electron.IpcRendererEvent,
        id: string,
      ...
      return () =>
        ipcRenderer.removeListener("ssh:reconnect-exhausted", handler);
    },
    ...
    onKBDIPrompt: (
      callback: (connectionId: string, prompt: unknown) => void,
    ) => {
      const handler = (
        _e: Electron.IpcRendererEvent,
        id: string,
  ...
  terminal: {
    ...
    write: (shellId: string, data: string) =>
      ipcRenderer.send("terminal:write", shellId, data),
  ...
  sftp: {
    open: (connectionId: string) =>
      ipcRenderer.invoke("sftp:open", connectionId) as Promise<{
        success: boolean;
        error?: string;
      }>,
    close: (connectionId: string) =>
      ipcRenderer.invoke("sftp:close", connectionId),
    readdir: (connectionId: string, path: string) =>
      ipcRenderer.invoke("sftp:readdir", connectionId, path) as Promise<{
        success: boolean;
        data?: unknown[];
        error?: string;
      }>,
    ...
    realpath: (connectionId: string, path: string) =>
      ipcRenderer.invoke("sftp:realpath", connectionId, path) as Promise<{
        success: boolean;
        data?: string;
        error?: string;
      }>,
    ...
    localReaddir: (path: string) =>
      ipcRenderer.invoke("sftp:local-readdir", path) as Promise<{
        success: boolean;
        data?: unknown[];
        error?: string;
      }>,
  ...
  clientkey: {
    ...
    remove: (id: string) =>
      ipcRenderer.invoke("clientkey:remove", id) as Promise<boolean>,
    ...
    getPublicKey: (id: string) =>
      ipcRenderer.invoke("clientkey:getPublicKey", id) as Promise<
        string | null
      >,
  },
  ...
  snippets: {
    ...
    getCategories: () =>
      ipcRenderer.invoke("snippet:getCategories") as Promise<string[]>,
  },
  ...
  portforward: {
    ...
    list: (connectionId: string) =>
      ipcRenderer.invoke("portforward:list", connectionId) as Promise<
        unknown[]
      >,
  },
  ...
  sql: {
    ...
    cancelAllQueries: (sqlSessionId: string) =>
      ipcRenderer.invoke("sql:cancelAllQueries", sqlSessionId) as Promise<{
        success: boolean;
        error?: string;
      }>,
    ...
    getAllColumns: (sqlSessionId: string) =>
      ipcRenderer.invoke("sql:getAllColumns", sqlSessionId),
    ...
    configGet: (sessionId: string) =>
      ipcRenderer.invoke("sql:config:get", sessionId) as Promise<{
        success: boolean;
        data?: Record<string, any>;
        error?: string;
      }>,
    configSave: (config: Record<string, unknown>) =>
      ipcRenderer.invoke("sql:config:save", config) as Promise<{
        success: boolean;
        error?: string;
      }>,
    configDelete: (sessionId: string) =>
      ipcRenderer.invoke("sql:config:delete", sessionId) as Promise<{
        success: boolean;
        error?: string;
      }>,
    configGetStandalone: () =>
    ...
    onQueryStarted: (
      callback: (queryId: string, sqlSessionId: string, query: string) => void,
    ) => {
      const handler = (
        _e: Electron.IpcRendererEvent,
        queryId: string,
    ...
    onQueryCompleted: (
      callback: (queryId: string, sqlSessionId: string) => void,
    ) => {
      const handler = (
        _e: Electron.IpcRendererEvent,
        queryId: string,
  ...
  monitor: {
    ...
    getHistory: (connectionId: string) =>
      ipcRenderer.invoke("monitor:getHistory", connectionId) as Promise<
        unknown[]
      >,
    getLatest: (connectionId: string) =>
      ipcRenderer.invoke("monitor:getLatest", connectionId) as Promise<
        unknown | null
      >,
  ...
  hostkey: {
    ...
    onVerifyRequest: (
      ...
      return () =>
        ipcRenderer.removeListener("hostkey:verify-request", handler);
    },
    respondVerify: (
      connectionId: string,
      response: {
        action: "trust-once" | "trust-save" | "accept-new" | "disconnect";
      },
    ) => {
