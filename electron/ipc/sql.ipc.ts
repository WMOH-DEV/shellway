...
const tunnelMap = new Map<
  string,
  { connectionId: string; ruleId: string; localPort: number }
>();
...
export function registerSQLIPC(): void {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      if (!config.type || !["mysql", "postgres"].includes(config.type)) {
        return {
          success: false,
          error: "Invalid database type — must be mysql or postgres",
        };
      }
      const validatedPort = Number(config.port);
      if (
        !Number.isFinite(validatedPort) ||
        validatedPort < 1 ||
        validatedPort > 65535
      ) {
        return {
      ...
      try {
        ...
        if (config.useSSHTunnel) {
          if (config.sshConfig) {
            ...
            const entry = await getPortForwardService().add(conn, {
              id: ruleId,
              type: "local",
              name: `SQL: ${config.database || "server"}`,
            ...
            if (entry.status === "error") {
              return {
                success: false,
                error: `SSH tunnel failed: ${entry.error}`,
              };
            }
            ...
            return {
              success: false,
              error: "SSH tunnel requested but no SSH configuration provided",
            };
          }
        }
        ...
        if (result.success) {
          return {
            success: true,
            tunnelPort:
              tunnelMap.get(sqlSessionId)?.localPort ||
              ephemeralSSH.get(sqlSessionId)?.localPort,
  ...
  ipcMain.handle("sql:getDatabases", async (_event, sqlSessionId: string) => {
    try {
      return {
        success: true,
        data: await sqlService.getDatabases(sqlSessionId),
      };
    } catch (err: any) {
  ...
  ipcMain.handle("sql:getTables", async (_event, sqlSessionId: string) => {
    try {
      return { success: true, data: await sqlService.getTables(sqlSessionId) };
    } catch (err: any) {
      return { success: false, error: err.message };
    }
  });
  ...
  ipcMain.handle("sql:getAllColumns", async (_event, sqlSessionId: string) => {
    try {
      return {
        success: true,
        data: await sqlService.getAllColumns(sqlSessionId),
      };
    } catch (err: any) {
      return { success: false, error: err.message };
    }
  ...
  ipcMain.handle(
    "sql:getColumns",
    async (_event, sqlSessionId: string, table: string, schema?: string) => {
      try {
        return {
          success: true,
          data: await sqlService.getColumns(sqlSessionId, table, schema),
        };
      } catch (err: any) {
        return { success: false, error: err.message };
      }
  ...
  ipcMain.handle(
    ...
    async (_event, sqlSessionId: string, table: string, schema?: string) => {
      try {
        return {
          success: true,
          data: await sqlService.getIndexes(sqlSessionId, table, schema),
        };
      } catch (err: any) {
        return { success: false, error: err.message };
      }
  ...
  ipcMain.handle(
    ...
    async (_event, sqlSessionId: string, table: string, schema?: string) => {
      try {
        return {
          success: true,
          data: await sqlService.getForeignKeys(sqlSessionId, table, schema),
        };
      } catch (err: any) {
        return { success: false, error: err.message };
      }
  ...
  ipcMain.handle(
    "sql:getTableStructure",
    async (_event, sqlSessionId: string, table: string, schema?: string) => {
      try {
        return {
          success: true,
          data: await sqlService.getTableStructure(sqlSessionId, table, schema),
        };
      } catch (err: any) {
        return { success: false, error: err.message };
      }
  ...
  ipcMain.handle(
    ...
    async (_event, sqlSessionId: string, table: string, schema?: string) => {
      try {
        return {
          success: true,
          data: await sqlService.getTableRowCount(sqlSessionId, table, schema),
        };
      } catch (err: any) {
        return { success: false, error: err.message };
      }
  ...
  ipcMain.handle(
    ...
    async (_event, sqlSessionId: string, table: string, schema?: string) => {
      try {
        return {
          success: true,
          data: await sqlService.getPrimaryKeyColumns(
            sqlSessionId,
            table,
            schema,
          ),
    ...
    async (
      ...
      try {
        if (opts.scope === "table" && opts.table) {
          const result = await transferService.exportTable(
            sqlSessionId,
            opts.table,
            filePath,
            opts,
          ...
          const result = await transferService.exportDatabase(
            sqlSessionId,
            filePath,
            opts,
          );
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        const result = await transferService.importSQL(
          sqlSessionId,
          filePath,
          options as ImportSQLOptions,
        );
        return { success: true, operationId: result.operationId };
      } catch (err: any) {
        return { success: false, error: err.message || String(err) };
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        const result = await transferService.importCSV(
          sqlSessionId,
          filePath,
          options as ImportCSVOptions,
        );
        return { success: true, operationId: result.operationId };
      } catch (err: any) {
        return { success: false, error: err.message || String(err) };
    ...
    async (
      ...
      if (!sshClient) {
        return {
          success: false,
          error:
            "Backup requires an SSH connection. Direct database connections do not support server-side backup.",
        };
      }
      ...
      const opts = (options || {}) as BackupOptions & {
        dbHost?: string;
        dbPort?: number;
        dbUser?: string;
        dbPassword?: string;
      ...
      if (!dbConfig.user) {
        return {
          success: false,
          error:
            "Database username is required for backup (provide dbUser in options)",
        };
      }
    ...
    async (
      ...
      if (!dbConfig.user) {
        return {
          success: false,
          error:
            "Database username is required for restore (provide dbUser in options)",
        };
      }
  ...
  ipcMain.handle(
    "sql:createDatabase",
    async (_event, sqlSessionId: string, options: unknown) => {
      if (!sqlSessionId || typeof sqlSessionId !== "string") {
        return { success: false, error: "Invalid session ID" };
      }
      if (!options || typeof options !== "object") {
      ...
      try {
        return await transferService.createDatabase(
          sqlSessionId,
          opts as {
            name: string;
            charset?: string;
  ...
  ipcMain.handle(
    "sql:getCollations",
    async (_event, sqlSessionId: string, charset: string) => {
      if (!sqlSessionId || typeof sqlSessionId !== "string") {
        return { success: false, error: "Invalid session ID" };
      }
      ...
      try {
        if (dbType === "mysql") {
          ...
          const collations = result.rows.map(
            (row: Record<string, unknown>) => row["Collation"] as string,
          );
          ...
          const collations = result.rows.map(
            (row: Record<string, unknown>) => row["collname"] as string,
          );
          return { success: true, data: collations };
        }
      } catch (err: any) {
  ...
  ipcMain.handle(
    ...
    async (_event, sqlSessionId: string, statements: string[]) => {
      if (!sqlSessionId || typeof sqlSessionId !== "string") {
        return { success: false, error: "Invalid session ID" };
      }
  ...
  ipcMain.handle(
    ...
    async (_event, sqlSessionId: string, table: string, schema?: string) => {
      ...
      try {
        const ddl = await transferService.generateCreateTable(
          sqlSessionId,
          table,
          schema,
        );
...
async function createEphemeralTunnel(
  ...
  if (sshConfig.authMethod === "privatekey") {
    if (!sshConfig.privateKeyPath) {
      throw new Error(
        "Private key file path is required for key-based SSH authentication",
      );
    }
    const { existsSync } = await import("fs");
    if (!existsSync(sshConfig.privateKeyPath)) {
      throw new Error(
        `Private key file not found: ${sshConfig.privateKeyPath}`,
      );
    }
  }
