...
function buildDataQuery(opts: {
  ...
  if (sortColumn) {
    ...
    const pkOrder = primaryKeyColumns
      .map((col) => `${quoteIdentifier(col, dbType)} ASC`)
      .join(", ");
...
export const DataTabView = React.memo(function DataTabView({
  ...
  useEffect(() => {
    ...
    const initTimer = setTimeout(() => {
      ...
      (async () => {
        try {
          const res = await (window as any).novadeck.sql.getTableStructure(
            sqlSessionId,
            table,
            schema,
          );
            ...
            for (const fk of fks) {
              for (let i = 0; i < fk.columns.length; i++) {
                fkMap[fk.columns[i]] = {
                  ...
                  referencedColumn:
                    fk.referencedColumns[i] || fk.referencedColumns[0],
                };
              }
            }
    ...
    try {
      ...
      for (const merged of mergedUpdates.values()) {
        ...
        for (const [col, { new: newVal }] of Object.entries(merged.changes)) {
          ...
          if (
            ...
            newVal.startsWith(SQL_EXPR_PREFIX)
          ) {
      ...
      for (const change of currentTableChanges) {
        ...
        const entries = Object.entries(change.newRow).filter(
          ([col, val]) =>
            !(autoIncrCols.has(col) && (val === null || val === undefined)),
        );
  ...
  const resultWithInserts = useMemo<QueryResult | null>(() => {
    ...
    const insertRows = insertChanges.map(
      (c) => c.newRow as Record<string, unknown>,
    );
    return {
      ...result,
